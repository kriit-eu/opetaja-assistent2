...
class HighlightFinalGradesFeature extends BaseFeature {
  ...
  if (Logger.isDebugMode()) Logger.info(`✨ [HighlightFinalGradesFeature] Row ${rowIdx} has ${cells.length} cells`)
      finalGradeCols.forEach(colIdx => {
        ...
        if (cell) {
          if (Logger.isDebugMode()) Logger.info(`✨ [HighlightFinalGradesFeature] Highlighting FINAL cell at row ${rowIdx}, col ${colIdx}, value: "${cell.textContent.trim()}"`)
      ...
      ovCols.forEach(colIdx => {
        const cell = cells[colIdx]
        if (cell) {
          if (Logger.isDebugMode()) Logger.info(`✨ [HighlightFinalGradesFeature] Highlighting ÕV cell at row ${rowIdx}, col ${colIdx}, value: "${cell.textContent.trim()}"`)
          if (!cell.textContent.trim()) {
            if (shouldHighlight) {
              if (inWarningWindow) {
