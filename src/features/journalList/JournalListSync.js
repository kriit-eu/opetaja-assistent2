...
class JournalListSyncFeature extends BaseFeature {
  ...
  async syncWithKriit() {
    ...
    try {
      ...
      const failedSyncs = []
      const touchedFinalGradeJournalIds = new Set()
      ...
      try {
        ...
        for (let bi = 0; bi < batches.length; bi++) {
          ...
          try {
            ...
            try {
              ...
              touchedFinalGradeJournalIds.add(batch.journalId)
              ...
              if (!isAssignmentLevelOnly && studentsToUpdate.length > 0) {
                ...
                if (syncedGrades.length > 0) {
                  try {
                    ...
                    Logger.error('Failed to notify Kriit about synced grades after successful Tahvel write:', error)
                  }
                }
              }
        ...
        const finalGradeUpdates = await recalculateFinalGradesForTouchedJournals(this.api, touchedFinalGradeJournalIds)
        ...
        if (failedSyncs.length === 0) {
          ...
          if (actualUpdates > 0 || assignmentLevelUpdates > 0) {
            ...
            if (finalGradeSummary) successMessage += ` ${finalGradeSummary}`
          ...
          if (finalGradeSummary) this.showSuccessBanner(finalGradeSummary)
