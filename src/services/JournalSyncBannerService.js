...
export class JournalSyncBannerService {
  ...
  showMissingApiKeyBanner(onOpenSettings = null) {
    ...
    const banner = domService.createAndInsertElement('div', {
    ...
    }, 'Kriit API võti puudub', banner)

    // Add message
    domService.createAndInsertElement('p', {
      classList: ['ta-sync-message'],
    }, 'Kriit API võti on vajalik, et sünkroniseerida hindeid Tahvli ja Kriidi vahel. Palun sisestage API võti laienduse seadetes.', banner)
  ...
  showAllInSyncBanner(onRefresh = null, onClose = null) {
    ...
    }, 'Kõik hinded on sünkroonis!', banner)

    // Add message
    domService.createAndInsertElement('p', {
      classList: ['ta-sync-message'],
      style: 'margin: 0 0 15px 0;',
    }, 'Tahvli ja Kriidi vahel pole erinevusi. Kõik hinded on juba õigesti sünkroniseeritud.', banner)
  ...
  showDifferencesBanner(totalDifferences, onSync = null, onRefresh = null, renderDifferences = null) {
    ...
    domService.createAndInsertElement('h2', {
      classList: ['ta-sync-title'],
    }, 'Sünkroniseerimata hinded', banner)
    ...
    if (totalDifferences > 0 && onSync) {
      ...
      }, `Sünkroniseeri ${totalDifferences} hinnet Kriidist Tahvlisse`, banner)
    } else if (onRefresh) {
  ...
  _addSyncErrorActions(banner, error, options) {
    if (error && error.includes('403')) {
      if (error.includes('Permission denied') || error.includes('rights to modify')) {
        ...
        }, 'Teil puuduvad õigused selle päeviku muutmiseks. Sünkroniseerimine Tahvlisse on võimalik ainult päeviku õpetajal.', banner)

        domService.createAndInsertElement('p', {
          classList: ['ta-sync-error-help'],
        }, 'Tahvel lubab päeviku hindeid muuta ainult päeviku õpetajal. Kui te pole selle päeviku õpetaja, siis ei saa te hindeid sünkroniseerida.', banner)
        ...
        domService.createAndInsertElement('p', {
          classList: ['ta-sync-error-help'],
        }, 'See on autentimise viga. Palun kontrollige oma Kriit API võtit.', banner)
        ...
        if (options.onSettings) {
          domService.createAndInsertElement('button', {
            ...
            style: 'margin-right: 10px;',
          }, 'Lähtesta Kriit API võti', banner)
      ...
      domService.createAndInsertElement('p', {
        classList: ['ta-sync-error-help'],
      }, 'See funktsioon vajab päeviku linke lehel. Palun veenduge, et olete päevikute nimekirja lehel.', banner)
      ...
      if (error.includes('is not enrolled in this assignment')) {
        helpText = 'Õpilane on leitud Tahvlis, kuid ta ei ole selle ülesande nimekirjas. Sünkroniseerimine ei ole võimalik, kuna õpilane ei ole selle ülesande osalejate nimekirjas. See võib juhtuda, kui õpilane on Kriidis, kuid ei ole selle konkreetse ülesande nimekirjas Tahvlis.'
      } else if (error.includes('Could not find student with personal code') ||
                error.includes('Refusing to update a different student')) {
        helpText = 'Õpilase isikukoodi ei leitud Tahvlis. Sünkroniseerimine ei ole võimalik, kuna me ei saa tuvastada, millise õpilase hinnet tuleks muuta. See võib juhtuda, kui õpilane on Kriidis, kuid mitte Tahvlis, või kui isikukoodid on erinevates formaatides.'
      }
      ...
      domService.createAndInsertElement('p', {
        classList: ['ta-sync-error-help'],
      }, 'Proovige lehte värskendada või puhastada vahemälu.', banner)
